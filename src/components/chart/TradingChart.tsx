...
const TradingChartComponent: React.FC<TradingChartProps> = ({
  ...
  // Memoize unique sorted candles to avoid re-sorting on every render
  ...
  const { ema9Data, ema21Data } = useMemo(() => {
    ...
    const data9: LineData<Time>[] = new Array(ema9Vals.length);
    for (let i = 0; i < ema9Vals.length; i++) {
      data9[i] = {
        time: uniqueCandles[i + offset9].time as Time,
        value: ema9Vals[i],
      };
    }

    const data21: LineData<Time>[] = new Array(ema21Vals.length);
    for (let i = 0; i < ema21Vals.length; i++) {
      data21[i] = {
        time: uniqueCandles[i + offset21].time as Time,
        value: ema21Vals[i],
      };
    }
  ...
  useEffect(() => {
    ...
    if (showSMC && smcResult) {
      ...
      for (let i = 0; i < smcResult.events.length; i++) {
        ...
        if (seenTimes.has(ev.breakTime)) {
          markers.push({
            time: ev.breakTime as Time,
      ...
      for (let i = 0; i < smcResult.liquidity.length; i++) {
        ...
        if (liq.swept && liq.sweepTime && seenTimes.has(liq.sweepTime)) {
          markers.push({
            time: liq.sweepTime as Time,
            position: liq.type.includes("BUY") || liq.type.includes("HIGH") ? "aboveBar" : "belowBar",
            color: "#F59E0B",
            shape: "circle",
            text: "SWEEP",
          });
    ...
    // Clear previous price lines before creating new ones to prevent leaks
    ...
    if (showTargets && aiAnalysis && aiAnalysis.signal !== "NO_TRADE" && candleSeriesRef.current) {
        ...
        <div className="flex items-center gap-3 text-[11px] text-neutral-400">
          <button
            ...
            className={`flex items-center gap-1 px-1.5 py-0.5 rounded transition-colors ${
              showEMAs ? "text-emerald-400 bg-emerald-950/30" : "text-neutral-600 hover:text-neutral-400"
            }`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-1 px-1.5 py-0.5 rounded transition-colors ${
              showSMC ? "text-sky-400 bg-sky-950/30" : "text-neutral-600 hover:text-neutral-400"
            }`}
          >
          ...
          {aiAnalysis && aiAnalysis.signal !== "NO_TRADE" && (
            <button
              ...
              className={`flex items-center gap-1 px-1.5 py-0.5 rounded transition-colors ${
                showTargets ? "text-amber-400 bg-amber-950/30" : "text-neutral-600 hover:text-neutral-400"
              }`}
            >
